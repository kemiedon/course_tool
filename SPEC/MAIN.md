...
- **內容結構** (v1.6.0 更新)：
  - **第一段（痛點引入）**：30-40 字，直擊目標客群痛點
  - **第二段（課程資訊）**：40-50 字，包含開課日期、上課時間、課程天數、總時數
  - **第三段（學習目標）**：80-100 字，列出 3-5 個主要學習目標
  - **第四段（解決方案與成果）**：80-100 字，說明課程如何解決痛點及學完後的具體能力
- **自動整合資訊** (v1.6.0 新增)：
  - 課程開始日期與結束日期
  - 上課時間區間
  - 從課綱自動提取前 5 個主要學習目標
  - 課程天數與時數統計
- 字數: 250-300 字（含課程時間與學習目標）

#### 3.2 編輯功能

- 可手動編輯 AI 生成的宣傳內容
- 提供「重新生成」按鈕
- 字數統計（建議 250-300 字，達標時顯示綠色）
- 複製到剪貼簿功能
- 圖示提示：自動整合課程日期、時間、學習目標與課綱重點
