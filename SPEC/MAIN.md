...
> 💡 **步驟調整**：宣傳內容生成已移至資訊圖表之後（步驟 4），以便整合完整課程資訊

#### 3.1 宣傳介紹 (v1.6.3 全新語氣與架構)

- 使用 Gemini API 生成課程宣傳文案
- **語氣定位**：像在跟家長聊天，有溫度、有同理心，不是推銷
- **內容架構**（完全參考提供的範本）：
  1. **擔心引入**：用家長的擔心作為開頭（「如果你開始擔心...」）
  2. **問題描述**：具體描述孩子目前的學習困境，有畫面感
  3. **工具定位**：點出關鍵不是工具本身，而是如何運用
  4. **課程差異**：說明這門課的獨特定位
  5. **學習成果**：列出孩子會一步步學會的能力（✅ 格式）
  6. **影響層面**：說明這些能力會如何影響孩子（✔ 格式）
  7. **實作說明**：透過大量實作與引導達成目標
  8. **課程資訊**（必須包含，使用 emoji）：
     - 📅 開課日期（格式：YYYY / MM / DD）
     - 🕘 課程時數
     - 💰 課程費用（選填）
     - 👨‍💻 上課方式
     - 👨‍👩‍👧‍👦 適合對象（3 個 ✅ 條列）
  9. **溫暖結尾**：鼓勵家長參與孩子的學習轉變

- **自動整合資訊** (v1.6.3 更新)：
  - 從課綱自動提取學習目標（改寫為「孩子會學會」格式）
  - 開課日期（簡潔格式：2026 / 01 / 03）
  - 上課時間與總時數
  - 根據目標客群改寫適合對象
...
11. 生成每日資訊圖表（顯示進度）
    ↓
12. AI 生成課程宣傳介紹（整合圖表與課綱）
    ↓
13. 檢視/編輯宣傳內容
    ↓
14. 完成！儲存課程
    ↓
15. [僅兒童課程] 可選：生成 Google 報名表單
```
...
| Phase 3: 日曆與課綱   | 98%    | ✅ 近乎完成 |
| Phase 4: 視覺化與宣傳 | 98%    | ✅ 近乎完成 |
...
- ✅ 宣傳文案全新語氣與架構（參考家長視角範本）
- ✅ 必須包含：開課日期、課程時數、課程費用、適合對象
- ✅ 學習目標改寫為「孩子會學會」的具體能力描述
- ✅ 加入課程費用參數（選填）
- ✅ 調整步驟順序：資訊圖表 → 宣傳內容（便於整合完整資訊）
- ✅ 優化日期格式為 YYYY / MM / DD
...
- ✅ 課程時間資訊卡片顯示（開始日期、結束日期、上課時間）
- ✅ CurriculumEditor 加入課程時間資訊卡片
- ✅ InfographicGenerator 加入課程時間資訊卡片
- ✅ 響應式設計（手機 1 欄，桌面 3 欄）
- ✅ 日期格式化（中文日期 + 星期）
- ✅ 課程統計顯示（總天數、每日時數、總時數）
